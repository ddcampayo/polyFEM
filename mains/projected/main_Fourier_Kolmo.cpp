...
void load_force_on_fft(const Triangulation& T , CH_FFT& fft  );
...
void load_force_on_fft( const Triangulation& T , CH_FFT& fft  ) {
  ...
  c_array f0x( Nb , Nb , align );
  c_array f0y( Nb , Nb , align );
  ...
  for(F_v_it vit=T.vertices_begin();
    ...
    Vector_2 f0 =  vit->force.val();
    f0x(i,j) = f0.x();
    f0y(i,j) = f0.y();

  }

  fft.set_force( f0x , f0y );
